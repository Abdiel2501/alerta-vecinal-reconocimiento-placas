# -*- coding: utf-8 -*-
"""
run_original_telegram_v8.py — Motor de IA V8
═══════════════════════════════════════════════════════════════
NUEVO en V8:

  [1] PaddleOCR (Cambio Crítico de Motor de Lectura)
      Se reemplazó EasyOCR por PaddleOCR. Es mucho más rápido, 
      robusto y preciso para detectar caracteres delgados (como
      los de las placas mexicanas) sin confundirse tanto.

  [2] Filtro Dinámico de Brillo (Día / Noche)
      La IA analiza el brillo promedio de la placa.
      - Si está muy brillante (reflejo de sol): oscurece y aplica unsharp masking.
      - Si está muy oscuro (noche): aplica ecualización de luz y filtro bilateral antiruido.
      - Si es normal: contraste equilibrado.

  [3] Validación de Formato Estricto (Filtro Antibasura)
      Rechaza automáticamente cualquier lectura de PaddleOCR que:
      - Tenga menos de 5 o más de 7 caracteres.
      - Sean puras letras (ej. leyó "TAMAULIPAS").
      - Sean puros números.
      Esto garantiza que el sistema de votos solo reciba placas válidas.

Heredado de V7:
  - Pipeline 100% en Hilo de Fondo (FPS fluidos)
  - Re-identificación de Vehículos (ReID por histograma)
  - Limpiador posicional y corrección de perspectiva
  - Filtro estricto de Falsos Positivos de BBox (>45% confianza)
"""
import cv2
import time
import csv
...
# ─────────────────────────────────────────────────────────────────────
# Preprocesamiento V8 (Filtro Dinámico de Brillo)
# ─────────────────────────────────────────────────────────────────────
...
def preprocesamiento_dinamico(roi_base, area):
    ...
    # 2. Análisis Dinámico de Brillo (V8)
    brillo_medio = cv2.mean(gray)[0]
    
    if brillo_medio > 170:
        # MUY BRILLANTE (Reflejo del sol). Reducimos luz fuerte.
        clahe = cv2.createCLAHE(clipLimit=4.0, tileGridSize=(6,6))
        eq = clahe.apply(gray)
        bl = cv2.GaussianBlur(eq, (5,5), 1.5)
        res = cv2.addWeighted(eq, 1.6, bl, -0.6, 0) # Unsharp masking fuerte
    elif brillo_medio < 70:
        # MUY OSCURO (Noche o sombra). Subimos luz y quitamos ruido.
    ...
    # Convertimos a BGR para PaddleOCR (espera 3 canales)
    res_bgr = cv2.cvtColor(res, cv2.COLOR_GRAY2BGR)
    return res_bgr

# ─────────────────────────────────────────────────────────────────────
# [V8] Formateador y Lector (PaddleOCR)
# ─────────────────────────────────────────────────────────────────────

def validar_formato_placa(texto):
    """Filtra basura. Retorna texto limpio si es placa válida, o cadena vacía si es basura."""
    # Quitar guiones y espacios
    texto = re.sub(r'[^A-Z0-9]', '', texto.upper())
    
    # Longitud de una placa típica de auto o moto
    ...
    # Si son puras letras (ej. TAMAULIPAS) o puros números (ej. un teléfono) -> basura
    if letras == 0 or numeros == 0:
        return ""
        
    # Las placas mexicanas de auto suelen tener max 4 letras o max 4 números.
    if letras > 4 or numeros > 4:
        return ""
        
    # Limpiador posicional para TAMAULIPAS (3 letras, 2 num, 2 letras/num)
    if re.match(r'^[A-Z]{3}[0-9]{2}[A-Z0-9][0-9]{2}$', texto):
        texto = texto[:5] + texto[6:]
        
    if len(texto) == 7:
        # Corrección de confusión O/0, S/5, etc. según posición
...
def _ocr_una_imagen(reader, img):
    ...
    if len(txt) < 5 or conf_prom < 0.6:
        try:
            res2 = _ocr_silencioso(reader, img, decoder='beamsearch', beamWidth=5, **base_kwargs)
            if res2:
                conf2 = sum(float(r[2]) for r in res2)/len(res2)
                txt2  = "".join(r[1].strip().upper().replace(" ","").replace("-","") for r in res2)
...
def leer_placa_completa(reader, roi_base, area):
    ...
    # EasyOCR Inferencia
    results = _ocr_una_imagen(reader, img_optima)
        
    if not results:
        return "", 0.0, img_optima
        
    lineas = sorted(results, key=lambda r: r[0][0][0])
    ...
    conf_prom = conf_acum / len(lineas)
...
class DeteccionOCRWorker:
    ...
    def _procesar(self, track_id, vehicle_roi, veh_bbox, frame_num):
        ...
        for plate_box in plate_results[0].boxes:
            ...
            # [V8] Margen uniforme y ajustado (12%)
            m = max(int(pw * 0.12), 4)
            lpx1c = max(0, lpx1 - m)
            lpy1c = max(0, lpy1 - m)
...
def main():
    ...
    output_video  = 'output_v8.mp4'
    classes_to_detect = [0,1,2,3,5]

    print("🤖 Cargando V8 (PaddleOCR + Dynamic Filter + Format Checker)...")
    # Inicializar EasyOCR (Compatible con YOLO/PyTorch)
    import torch
    usar_gpu = torch.cuda.is_available()
    print("⚡ GPU PyTorch/OCR: {'Sí' if usar_gpu else 'CPU'}")
    ...
    while cap.isOpened():
        if not paused:
                            ...
                            if conf_f >= 0.12 and texto_f:
                                ...
                                if prev is None or conf_f > prev.get('confidence',0.0):
                                    ...
                                    if area >= parea:
                                        ...
                                        try: cv2.imwrite(f'plates/{frame_number}_{track_id}_{texto_f}_V8.png', img_u)
                                        except: pass
            ...
            put_text(frame, "V8: PaddleOCR+Dynamic+Antibasura", (10,yo), color=(200,200,0), font_scale=0.45)
            out.write(frame)

        if frame is not None:
            cv2.imshow('Detection and Tracking V8', frame)
