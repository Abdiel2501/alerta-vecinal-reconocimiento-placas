# -*- coding: utf-8 -*-
"""
servidor_ia_v13.py — Plataforma SaaS Multiusuario de IA para Reconocimiento de Placas

Esta versión implementa una arquitectura SaaS Multi-tenant:
...
parser = argparse.ArgumentParser(description="Servidor IA SaaS AlertaVecinal — WebSocket")
...
class UserPipeline:
    """Encapsula la captura de video, el análisis IA V13 y la transmisión WebSocket
    ...
    def __init__(self, usuario_id: int, user_data: dict):
        ...
        # Motores internos de IA V13 para este usuario
    ...
    def _bucle_ia(self):
        ...
        print(f"[User {self.usuario_id}] 🚀 Loop de IA V13 iniciado.")
            ...
            try:
                ...
                if results and results[0].boxes.id is not None:
                    ...
                    for box, track_id, conf_v in zip(ultimas_cajas, ultimos_ids_rastreo, ultimas_confianzas):
                        ...
                        # Gemini V13 asíncrono si está habilitada su API Key
                        vot = self.votadores.get(track_id)
...
app = FastAPI(title="AlertaVecinal SaaS Server V13")
...
async def websocket_saas(websocket: WebSocket, token: str):
    ...
    try:
        ...
        # Opcional: Detener la captura de la cámara si ya no hay ningún cliente visualizándola para ahorrar CPU
        if n_restantes == 0:
            print(f"[Orquestador] Deteniendo pipeline inactivo de usuario {usuario_id} (0 clientes conectados).")
            estado_servidor_saas.detener_pipeline(usuario_id)
...
# ─── Utilidades PTZ, Imagen y Formato ─────────────────────────────────────────
...
async def _on_startup():
    ...
    # (Opcional) Autoiniciar pipelines para todos los usuarios con cámaras al arrancar el servidor
    try:
...
if __name__ == "__main__":
    print("=" * 60)
    print("  🖥️  AlertaVecinal — Servidor IA SaaS WebSocket V13")
    print("=" * 60)
