# -*- coding: utf-8 -*-
"""
run_original_telegram_v7.py — Motor de IA V7
═══════════════════════════════════════════════════════════════
NUEVO en V7:

  [1] FIX DEL CUADRO BLANCO (Falsos Positivos)
      El modelo de placas a veces confundía logotipos o reflejos
      del parabrisas con placas. Ahora se filtra estrictamente por
      confianza (Solo > 45%).
      Además, los márgenes gigantes asimétricos se eliminaron para
      usar un margen ajustado del 12% en todos los bordes, evitando
      capturar ruido del vehículo.

  [2] MEJORA OCR PARA LETRAS (XLK, WZT...)
      Se eliminó la lectura por zonas (que podía cortar letras a la mitad
      si la placa estaba inclinada). En su lugar:
      - Se usan 2 filtros globales de altísimo contraste (CLAHE + Bilateral).
      - Se añadió `mag_ratio=2.0` nativo a EasyOCR, lo que hace un upscaling
        interno de los caracteres, resolviendo el problema de las letras delgadas.

  [3] RENDIMIENTO MANTENIDO (15-20 FPS)
      El pipeline asíncrono (Worker) sigue activo.

Heredado de V6:
  - Pipeline 100% en Hilo de Fondo
...
def super_resolver(roi, area):
    ...
    if area >= 3000:
        sc = T / h
        return cv2.resize(roi, (max(1,int(w*sc)), max(1,int(h*sc))), interpolation=cv2.INTER_LANCZOS4)
    elif area >= 800:
        sc = T / h
        base = cv2.resize(roi, (max(1,int(w*sc)), max(1,int(h*sc))), interpolation=cv2.INTER_LANCZOS4)
...
def _ocr_una_imagen(reader, img):
    # En V7 usamos mag_ratio=2.0 para mejorar caracteres delgados/pequeños
    base_kwargs = dict(
...
def leer_placa_completa(reader, roi_base, area):
    ...
    h, w = roi.shape[:2]
    if h == 0 or w == 0: return "", 0.0, roi

    # Variante 1: Color unsharp (ideal general)
    bl = cv2.GaussianBlur(roi, (5,5), 1.5)
    v1 = cv2.addWeighted(roi, 1.5, bl, -0.5, 0)
    
    # Variante 2: Grises CLAHE + Bilateral (excelente contraste)
    gray = cv2.cvtColor(roi, cv2.COLOR_BGR2GRAY)
    clahe = cv2.createCLAHE(clipLimit=3.5, tileGridSize=(6,6))
    v2 = cv2.bilateralFilter(clahe.apply(gray), d=11, sigmaColor=80, sigmaSpace=80)

    mejor_texto, mejor_conf, mejor_img = "", 0.0, roi

    for img_variante in [v1, v2]:
        res = _ocr_una_imagen(reader, img_variante)
        if not res: continue
        ...
        if len(txt) >= 4 and conf > mejor_conf:
            mejor_texto, mejor_conf, mejor_img = txt, conf, img_variante
...
class DeteccionOCRWorker:
    ...
    def _procesar(self, track_id, vehicle_roi, veh_bbox, frame_num):
        ...
        for plate_box in plate_results[0].boxes:
            ...
            # [V7] Filtrar falsos positivos (como logotipos o limpiaparabrisas)
            if conf_plate < 0.45:
                continue
            ...
            # [V7] Margen uniforme y ajustado (12%)
            m = max(int(pw * 0.12), 4)
            lpx1c = max(0, lpx1 - m)
            lpy1c = max(0, lpy1 - m)
...
def main():
    ...
    output_video  = 'output_v7.mp4'
    classes_to_detect = [0,1,2,3,5]

    print("🤖 Cargando V7 (Fix BBox falsos + mag_ratio + Async)...")
    import torch
    usar_gpu = torch.cuda.is_available()
    print(f"⚡ GPU: {'Sí CUDA' if usar_gpu else 'CPU'}")
    ...
    while cap.isOpened():
        if not paused:
                ...
                for box in result.boxes:
                    ...
                    if class_names[cls] in ["car","motorbike","bus"]:
                        ...
                        if res:
                            ...
                            # Dibujar rectángulo de placa (Color según tamaño para debug V7)
                            ...
                            if conf_f >= 0.12 and texto_f:
                                ...
                                if prev is None or conf_f > prev.get('confidence',0.0):
                                    ...
                                    if area >= parea:
                                        ...
                                        try: cv2.imwrite(f'plates/{frame_number}_{track_id}_{texto_f}_V7.png', img_u)
                                        except: pass
            ...
            put_text(frame, "V7: Fix BBox+Async+ReID+SR", (10,yo), color=(200,200,0), font_scale=0.45)
            out.write(frame)

        if frame is not None:
            cv2.imshow('Detection and Tracking V7', frame)
